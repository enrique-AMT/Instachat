...
class ChatsDAO:
    ...
    def getAllChats(self):
        ...
        # select chat_id, chat_name, owner_id, count(u_belongs) from instachat.chat natural inner join
        # instachat.belongs " \
    ...
    def insertUserToChat(self, chat_id, user_id):
        cursor = self.conn.cursor()
        cursor.execute("insert into instachat.belongs(u_belongs, c_user_belongs)"
                       "values(%s, %s);", [user_id, chat_id])
        self.conn.commit()
