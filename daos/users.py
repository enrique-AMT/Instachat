...
class UsersDAO:
  ...
  def getUserChats(self, user_id):
    print("DAO")
    cursor = self.conn.cursor()
    # cursor.execute("select chat_id, chat_name, owner_id from instfachat.chat where chat_id in (select c_user_belongs from "
  ...
  def createUser(self, first_name, last_name, u_email_address, u_password, username):
    ...
    cursor.execute("insert into instachat.user(first_name, last_name, u_email_address, u_password, username) "
                   "values(%s, %s, %s, crypt(%s, gen_salt('md5')), %s) returning user_id;",
  ...
  def login(self, username, password):
    cursor = self.conn.cursor()
    cursor.execute("select user_id from instachat.user where username = %s and u_password = crypt(%s, u_password)", [username, password])
    result = cursor.fetchone()
    return result
